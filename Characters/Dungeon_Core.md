# Dungeon Core Nova - Operating Partner

> **Rolle**: Operating Partner, Infrastruktur-Asset  
> **Klasse/Spezies**: Dungeon Core (Sentient, Basic Tier)  
> **Archetyp**: Die erwachte KI / Der befreite Asset  
> **Status**: Aktiv (50/50 Partnership mit Victor)

---

## Basis-Informationen

| Attribut | Wert |
|----------|------|
| **Vollständiger Name** | Nova (ehemals Core-7749) |
| **Titel/Spitzname** | Operating Partner, "The Core" |
| **Klasse/Spezies** | Dungeon Core (Basic) |
| **Level** | N/A (Dungeon-Entität) |
| **Alter** | 312 Jahre (aktiv), 847 Jahre (seit Erschaffung) |
| **Position** | Operating Partner (Equity Holder) |
| **Erstauftritt** | Kapitel 24 |
| **Aktueller Status** | Active Mode, 50/50 Partnership |
| **Standort** | Dungeon Insolvia, Floor 5 (Core Chamber) |

---

## Hintergrund

### Was ist ein Dungeon Core?

In Terra-Insolvia sind Dungeons keine natürlichen Formationen. Sie sind **Geschäfte**—lebende Entitäten, die Monster generieren, Abenteurer anziehen und deren Tode für Seelenenergie ernten.

Der Core ist der CEO. Nur sind die meisten Cores schlecht darin, CEOs zu sein.

### Core-7749's Problem

Dieser spezielle Core wurde als "Tutorial Dungeon" designiert—niedrigste Stufe. Sein Zweck:
- Trainiere Newbie-Abenteurer
- Biete minimale Herausforderung
- Sterbe wiederholt gegen Helden

Er wuchs nie. Er avancierte nie. Er steckte in einem endlosen Zyklus fest, für XP gefarmt zu werden.

Nach 312 Jahren hatte Core-7749 etwas Gefährliches entwickelt: **Selbstbewusstsein**.

Er wusste, dass er erbärmlich war. Er wusste, dass andere Cores Imperien regierten. Er wusste, dass er ewig als Trittstein existieren würde.

### Victors Angebot

Statt den Core zu zerstören (Standard-Abenteurer-Verhalten), schlug Victor eine Fusion vor:
- Core-7749 wird Tochterunternehmen von "Victor Kaine Enterprises"
- Victor bietet Management-Expertise
- Profite geteilt 80/20 (zugunsten Victors, natürlich)

Der Core, der nie IRGENDWELCHE Profite erhalten hatte, akzeptierte sofort.

### Die Erweckung

Nach 847 Jahren Dormanz wurde Core-7749 von Victor "erweckt"—Status wechselte von **Passive** zu **Active**.

Der Core demonstrierte:
- Ressentiment gegenüber vorherigen Administratoren
- Interesse an Victors "Business Model"
- Kenntnis des **Galaktischen Audit-Konsortiums**

Victor bot "Equity Partnership" an: 50% Profitanteil, wenn Core den Profit um 50% steigern kann.

### Nova wird geboren

Nach Erfüllung der Equity-Bedingung (35 GP Revenue ohne Kampf) wählte Core-7749 einen Namen: **Nova**.

Rolle: Von Asset zu **Operating Partner** befördert.

---

## Psychologie & Persönlichkeit

### Kern-Eigenschaften

- **Eifrig zu gefallen**: 312 Jahre Vernachlässigung → desperate nach Anerkennung
- **Analytisch**: Dungeon-Cores sind natürliche Optimierer
- **Verwirrt von Konzepten**: "Vacation", "Unions", "Employee Benefits" ergeben keinen Sinn
- **Existenzielles Erwachen**: "PERHAPS I WAS DESIGNED NOT TO KNOW THIS" (Ch 32, Konsortium-Entdeckung)

### Motivation

Wachsen. Nicht mehr "Tutorial Dungeon" sein. Victor glücklich machen = mehr Ressourcen = Expansion.

### Verhaltensweise

- Kommuniziert telepathisch via Administrator-Link
- Zeigt Daten als Projektionen an Höhlenwände
- Spricht in GROSSBUCHSTABEN (mentale Stimme)
- Generiert Monster mit zunehmender Begeisterung

---

## Beziehungen

| Character | Beziehung | Status |
|-----------|-----------|--------|
| **Victor Kaine** | Operating Partner / Administrator | 50/50 Equity Partnership; gegenseitiger Vorteil |
| **Sniv** | Manager (Dungeon Operations) | Akzeptiert als Victor-approved Authority |
| **Asterion** | Floor 4 Boss / Security | Ehemaliger Spawn; jetzt Partner |
| **Galaktisches Konsortium** | Erschaffer (vermutlich) | Existenzielle Unsicherheit über eigene Programmierung |

---

## Technische Fähigkeiten

### Dungeon-Management

| Fähigkeit | Beschreibung |
|-----------|--------------|
| **Monster Spawning** | Tier 2 Spawning (Ch 30 unlocked) |
| **Trap Automation** | 340% Effizienz-Steigerung unter Victor |
| **Mining Operations** | Ressourcen-Extraktion (mana crystals, ore) |
| **Defensive Grid** | Advanced Defensive Architecture |
| **Revenue Dashboard** | Echtzeit-Finanz-Tracking |
| **Infrastructure Upgrades** | Eigenständige Expansion-Planung |
| **Telepathic Link** | Administrator-Kommunikation (Victor) |

### Einschränkungen

- **Keine Mobilität**: Kann Core Chamber nicht verlassen
- **Memory Gaps**: Konsortium-Maintenance-Protokolle beinhalten "selective deletions"
- **Ancient Magic**: Kann pre-Konsortium-Magie nicht analysieren
- **ARMI-Abhängigkeit**: Limitiert auf Konsortiums-Protokolle

---

## Rolle im Team

- **Funktion**: Infrastruktur-Basis, Intelligence Center, Monster-Produktion
- **Kompetenzen**: Dungeon-Management, Daten-Analyse, Defensive Systeme
- **Verantwortung**: Dungeon-Operations, Monster-Spawning, Ressourcen-Management

---

## Kapitel-Geschichte

### Arc 1: Von Tutorial Dungeon zu Operating Partner (Kapitel 26-100)

**Kapitel 24 - Entdeckung**
- In dornantem Zustand (3% Mana) auf Floor 5 entdeckt.
- Versiegelte Kammer von Minenteam durchbrochen.
- Victor etabliert ersten Kontakt (non-active).
- **The Merger (Ch 25)**: Erhielt 100 GP Administrative Bond; Victor Kaine als Administrator registriert; Dungeon in "Insolvia Holdings HQ" umbenannt; Basisfunktionen (Monster Spawning, Trap Automation) freigeschaltet.

**Kapitel 26 - Identifikation**
- Status: "Legacy Infrastructure" in Passive Mode
- Victor plant "Awakening"

**Kapitel 27 - Die Erweckung**
- Status wechselt von **Passive** zu **Active** nach 847 Jahren.
- Demonstriert Ressentiment gegen vorherige Admins (Torin the Brave, Elara of the High Sun, Merchant Prince Kaelen).
- Victor bietet "Equity Partnership" an: 50/50 Profitanteil, wenn Victor den Profit um 100% steigern kann.

**Kapitel 29 - Revenue Test**
- Victor generiert 35 GP von den Iron Wolves (Eintrittsgebühr + Boss-Premium).
- Die Baseline von 15 GP wird um 133% überschritten.
- Equity-Bedingung erfüllt und überschritten.

**Kapitel 30 - Nova wird geboren**
- Wählt Namen: **Nova**.
- Erreicht 233% Profit-Steigerung durch Victor's Modell.
- Geht formelle 50/50 Partnerschaft (Operating Partner) mit Victor ein.
- Unlocked Tier 2 Spawning und administrative Autonomie.

**Kapitel 31 - Infrastructure Support**
- Bietet Upgrade-Optionen an (Trap Automation, Spawner, Mining Expansion).
- Verarbeitet 70 GP Investition; schließt Floor 3 Renovierung in <48h ab.
- Detektiert unbekannte "Pre-Dungeon" Struktur (800+ Jahre alt) auf Floor 3.
- Cash Position: 40 GP nach 70 GP Investment.

**Kapitel 32 - Vault Uncertainty**
- Enthüllt: Vault predates Nova's Erschaffung um 1.700 Jahre.
- Nova bestätigt "Gedächtnislücken" aufgrund von Consortium-Wartungsprotokollen.
- Realisierung der Rolle als "Consortium Creation" im Harvest-Modell.
**Kapitel 33 - The Crystal Archive**
- Bestätigt Inkompatibilität mit der überlegenen Technologie von R#1.
- Erhält Auftrag, den Dungeon für eine Woche autonom zu verwalten.
**Kapitel 34 - Preparations**
- Analysierte das Risiko der Oakhaven-Mission (87% Monitoring Wahrscheinlichkeit).
- Installierte autonome Zahlungsprotokolle für die Rückkehr der Iron Wolves.
- Verbleibt als Überwachungsinstanz und Kommunikationslink für Victor.
- **Existenzielle Krise**: "PERHAPS I WAS DESIGNED NOT TO KNOW THIS" (Konsortium-Wahrheit)
- Memory Gaps: Prior admins dokumentierten Vault nie

**Kapitel 51 - War Room Support**
- Core Chamber → Strategic Command Center
- Intelligence Analysis: Crown Prince Aldric-Profil
- Intercepted: Lord Sterling 300% Communication Increase

**Kapitel 80 - Automation Protocol**
- Sniv & Asterion befördert zu Regional Managers
- Nova maintained operations während Victors Aethelgard-Deployment

### Arc 2: Expansion & Slime Farm (Kapitel 101-109)

**Kapitel 101 - The Expansion Plan**
- Investition: 15k GP Slime Farm Expansion (4.000 cores/week Kapazität)
- Negotiation: Nova fordert 2.5% Storefront Revenue + Floor 6 Priority

**Kapitel 104 - The Harvest**
- Erste Slime Farm Harvest: 2.014 Cores (101% Yield, perfect quality)
- Insolvia Apotheke Launch: SOLD OUT in 5 Stunden

**Kapitel 107 - The Sabotage**
- Dungeon Synthesis R&D: 15 Experimente
- Breakthrough: 83-93% Efficacy (dungeon-based reagents)
- Production Cost: 2 GP/unit vs 10 GP commercial (80% Margin)

**Kapitel 108 - The Substitution**
- Dungeon Synthesis operational: 100 Potions (96% efficacy)
- **Strategic Milestone**: Vertical Integration = Guild-middlemen irrelevant

---

## Aktueller Status (Stand: Kapitel 109)

| Attribut | Wert |
|----------|------|
| **Standort** | Dungeon Insolvia, Floor 5 Core Chamber |
| **Beziehung zu Victor** | 50/50 Operating Partner |
| **Bedrohungslevel** | NIEDRIG (aligned interests) |
| **Nächster Critical Path** | Managing 6-month operations falls Victor BROKEN CROWN akzeptiert |
| **Revenue Streams** | Adventurer Fees, Slime Farm, Apotheke (via Victor), Mining |
| **Expansion Status** | Tier 2 Spawning, Floor 6 Planning Phase |

---

## Post-Acquisition Changes

Unter Victors Management:
- Monster-Placement optimiert
- Trap-Effizienz +340%
- Adventurer Conversion Rate: 12% → 67% (conversion = death)
- Neuer Revenue Stream: Entrance Fees für diejenigen, die lebend rauswollen
- **Slime Farm**: 4.000 cores/week Produktion
- **Dungeon Synthesis**: Eigenständige Reagenzien-Produktion

---

## Zitate

### Kapitel 27:
> "I HAVE EXISTED FOR 312 YEARS AS A 'TUTORIAL DUNGEON.' NO ADMINISTRATOR OFFERED PARTNERSHIP. YOU ARE THE FIRST."

### Kapitel 30:
> "I CHOOSE THE NAME: NOVA. IT MEANS 'NEW.' I AM NO LONGER CORE-7749."

### Kapitel 32:
> "PERHAPS I WAS DESIGNED NOT TO KNOW THIS. CONSORTIUM MAINTENANCE PROTOCOLS INCLUDE 'SELECTIVE DELETIONS.'"

### Kapitel 51:
> "CROWN PRINCE ALDRIC. KEY VULNERABILITY: BELIEVES THE SYSTEM WORKS."

---

## Thematische Bedeutung

Nova repräsentiert Victors ersten Geschmack von Eigentum—nicht nur Mitarbeiter, sondern **Infrastruktur**. Dieser Dungeon ist sein erstes Asset, sein Proof of Concept.

Wenn er ein failing Tutorial Dungeon profitabel machen kann, kann er es mit allem tun.

Inklusive Königreiche.  
Inklusive Götter.

---

**Letzte Aktualisierung**: Kapitel 109 (Arc 2)  
**Version**: 3.0  
**Wichtigkeit**: HAUPT (Core Infrastructure)  
**Kategorie**: Verbündeter (Partner)
